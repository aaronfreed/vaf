...
- Transparent sides don’t align at all if there is no primary side defined for that surface. This occurs when a transparent texture is assigned to a side that has never been textured, whose reverse polygon’s ceiling is higher than or has the same height as its obverse polygon’s, and whose reverse polygon’s floor is lower than or has the same height as its obverse polygon’s.
  - On the other hand, when such a side *has* been textured (which can occur if the level was paved before the side’s reverse polygon was created, and/or if the heights of the polygons on either side have changed at some point), the texture that was applied to the primary side will render behind the intended transparent texture. “Undo” will not cause the primary side to stop rendering, either; the only way to do so is to remove the textures from the associated line in Weland. This issue probably cannot be fixed without some very complicated engine-side changes.
